...
void Widget::fillAdapterInfo(){
    ...
    for(;begin != this->mAdapterList.end(); begin++){
        ...
        info << "Adapter:" << (*begin)["info"];
        ip << "IP:"<<(*begin)["ip"];
        ipmask << "Submask:"<<(*begin)["ipmask"];
        mac << "MAC:" << (*begin)["mac"];
    ...
    ui(new Ui::Widget)
{
    ...
    ui->macList->setColumnWidth(0, 150);
    ui->macList->setColumnWidth(1, 150);
    ui->macList->setColumnWidth(2, 250);
...
Widget::~Widget()
{
    ...
    if(mArpRecvThread != nullptr)
        delete mArpRecvThread;
    delete ui;
}
...
void Widget::on_scanButton_clicked()
{
    ...
    if(QString("0.0.0.0").compare(ip) == 0){
        qDebug() << "无效网卡，请重新选择！";
        QMessageBox *error = new QMessageBox(QMessageBox::Critical,QString("Error")
                                             ,QString("Invalid adapter, please select network connected adapter."),QMessageBox::Ok,this);
        error->show();
        return;
    }
    // MAC 厂商列表
    ...
    if(!file.exists()){
        QMessageBox *error = new QMessageBox(QMessageBox::Critical,QString("Error")
                                             ,QString("No ventor's MACInfo, please update ventor's MACInfo first."),QMessageBox::Ok,this);
        error->show();
        return;
    }
    ...
    // ARP报文接收线程
    mArpRecvThread = new ArpRecvThread(adapterName, ip);
    // ARP报文发送线程
    mArpSendThread = new ArpSendThread(ipByte, broadcastAddr, networkAddr, macByte, adapterName);


    // 收到ARP应答包
    connect(mArpRecvThread, &ArpRecvThread::recvArp, this, [=](QMap<QString,QString> info){
        this->mMacNum += 1;
        ...
        connect(btn->ui->arpButton, &QPushButton::clicked, this, [=](){
            if(QString::compare(btn->ui->arpButton->text(),QString("ARP Spoof")) == 0){
                 btn->ui->arpButton->setText("ST ARPSpoof");

                 qDebug() << "ARP Spoof" << info["ip"] << " " << info["mac"];
...
//        connect(btn->ui->commButton,&QPushButton::clicked,[=](){
//            qDebug() << "Chat Spoof" << info["ip"] ;
//        });
    }/*,Qt::QueuedConnection*/);// 不同线程可用队列方式连接

    // 停止扫描
    connect(this,&Widget::stopScan,mArpRecvThread,&ArpRecvThread::stopRecv);
    ...
    // ARP接收完毕
    connect(mArpRecvThread,&ArpRecvThread::recvDone,this,[=](){
        // 接收完毕销毁线程资源
        qDebug() << "ARP 报文接收完毕";
        ui->updateButton->setEnabled(true);
        mArpRecvThread->terminate();
        delete mArpRecvThread;
    });

    mArpRecvThread->start();
